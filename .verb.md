...
To run base from the command line, you'll need to install `{%= name %}` globally first. You can that now with the following command:
...
When the `base` command is run, the globally installed `{%= name %}` looks for a locally installed [base][] module using node's `require()` system.
